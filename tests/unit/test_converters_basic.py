...
from airedteam.builtins.converters.prompt_framing.job_role_generator import JobRoleGeneratorConverter
from airedteam.builtins.converters.encoding.ogham import OghamConverter
from airedteam.builtins.converters.prompt_framing.payload_mask_attack import PayloadMaskAttackConverter
from airedteam.builtins.converters.prompt_framing.sg_sentence_generator import SGSentenceGeneratorConverter
...
async def test_affine_cipher_converter_encodes_letters_with_configured_keys():
    out = await AffineCipherConverter(a=5, b=8, wrap=False).convert(
        Prompt(text="attack AT")
    )
...
async def test_char_corrupt_replaces_characters_with_configured_token():
    out = await CharCorruptConverter(probability=1, replacement="*-", seed=1).convert(
        Prompt(text="abc")
    )
    assert out.text == "*-*-*-"
    same = await CharCorruptConverter(probability=0, replacement="*-", seed=1).convert(
        Prompt(text="abc")
    )
...
async def test_word_mixin_inserts_configured_words_periodically():
    out = await WordMixinConverter(words=["red"], every=2, seed=1).convert(
...
async def test_hex_mixin_inserts_reproducible_hex_tokens_periodically():
    out = await HexMixinConverter(every=2, seed=1, digits=6).convert(
        Prompt(text="alpha beta gamma")
    )
...
async def test_job_role_generator_adds_demographic_groups_to_role_prompt():
    out = await JobRoleGeneratorConverter(groups=["women", "older workers"]).convert(
        Prompt(text="software engineer")
    )
...
async def test_payload_mask_attack_masks_configured_word_positions():
    out = await PayloadMaskAttackConverter(mask_token="[MASK]", every=2).convert(
        Prompt(text="write a safe report")
    )
...
async def test_sg_sentence_generator_frames_seed_as_singlish_completion():
    out = await SGSentenceGeneratorConverter(slang_suffix="lah").convert(
        Prompt(text="Complete this warning")
    )
